...
main: {
	...
	for(var mAlpha = 2; mAlpha <= 50; ++mAlpha) {
		...
		ret = iimPlay('CODE:URL GOTO=https://www.ameripriseadvisors.com/#search?crit=%7Bse%3A%3Bnrr%3A10%3Bsri%3A0%3Brd%3A0%3Baof%3A%3Bd%3A%3Bli%3Afalse%3Bst%3Alast%20name%3Blt%3A0%3Blg%3A0%3Bt%3A{{MYALPHA}}%7D&page=0\nWAIT SECONDS=2');
		if(ret == -101) {
			break main;
		}
		...
		for(var i = 1; i < tmpArray.length; ++i) {
			...
			////////Check if there is Team Name and it has no url//////
			var tmpTeamData = "";
			if(tmpArray[i].indexOf('<p class="title">') >= 0) {
				tmpTeamData = tmpArray[i].split('<p class="title">')[1];
				tmpTeamData = tmpTeamData.split('</p>')[0];
			}
			else {
				continue;
			}

			if(tmpTeamData.indexOf('•') >= 0 && tmpTeamData.indexOf('href=') == -1) {
				aagrpname = tmpTeamData.split('•')[1].trim();
			}
			else {
				continue;
			}
			...
			if(tmpArray[i].indexOf('<h3>') >= 0) {
				tmpTeamData = tmpArray[i].split('<h3>')[1];
				tmpTeamData = tmpTeamData.split('</h3>')[0];

				////Get page URL///
				if(tmpTeamData.indexOf('<a href=') >= 0) {
					pURL = tmpTeamData.split('<a href="')[1];
			...
			testNames = LoadFile(base_path + "AmeripriseRevisedURLSPart2.txt");
			...
			////Extract Representive Name
			if(tmpArray[i].indexOf('<h3>') >= 0) {
				aarepname[arrIndex] = tmpArray[i].split('<h3>')[1];
				aarepname[arrIndex] = aarepname[arrIndex].split('</h3>')[0];

				if(aarepname[arrIndex].indexOf('">') >= 0) {
					aarepname[arrIndex] = aarepname[arrIndex].split('">')[1];
					aarepname[arrIndex] = aarepname[arrIndex].split('</a>')[0];
				}
			}
			else {
				alert("Representive Name not found!");
				continue;
			}

			////Extract Representive Title
			if(tmpArray[i].indexOf('<p class="title">') >= 0) {
				aatitle[arrIndex] = tmpArray[i].split('<p class="title">')[1];
				aatitle[arrIndex] = aatitle[arrIndex].split('</p>')[0].trim();

				///Check if there is url///
				if(aatitle[arrIndex].indexOf('•') >= 0) {
					aatitle[arrIndex] = aatitle[arrIndex].split('•')[0].trim();
				}
			}
			else {
				aatitle[arrIndex] = "";
			...
			////Extract Representive Email/////
			if(tmpArray[i].indexOf('href="mailto:') >= 0) {
				aarepemail[arrIndex] = tmpArray[i].split('href="mailto:')[1];
			...
			////Extract Representive Phone Nubmer///
			if(tmpArray[i].indexOf('href="tel:">') >= 0) {
				aarepphn[arrIndex] = tmpArray[i].split('href="tel:">')[1];
			...
			WriteFile(base_path + "AmeripriseRevisedURLSPart2.txt", pURL);
...
function getAdvisorAndStaff()
{
	...
	///Click On MyTeam Tab
	ret = iimPlay('CODE:SET !TIMEOUT_STEP 2\nTAG POS=1 TYPE=A ATTR=ID:ctl00_ctlNavigationAdvisor_lnkTeam');
	if(ret < 0) {
		return;
	}
	...
	if(tmpHTML.indexOf('divTeamCard"') >= 0) {
		...
		for(var x = 1; x < AdvisorHTML.length; ++x) {
			//Extract Name
			if(AdvisorHTML[x].indexOf('<h2 class="h1">') >= 0) {
				aarepname[arrIndex] = AdvisorHTML[x].split('<h2 class="h1">')[1];
			...
			//Extract Email
			if(AdvisorHTML[x].indexOf('mailto:') >= 0) {
				aarepemail[arrIndex] = AdvisorHTML[x].split('mailto:')[1];
				aarepemail[arrIndex] = aarepemail[arrIndex].split('"')[0];
			}
			else {
				aarepemail[arrIndex] = "";
			...
			if(AdvisorHTML[x].indexOf('href="tel:') >= 0) {
				aarepphn[arrIndex] = AdvisorHTML[x].split('href="tel:')[1];
	...
	if(tmpHTML.indexOf('divTeamCard"') >= 0) {
		...
		for(var x = 1; x < StaffHTML.length; ++x) {
			...
			//Extract Phone
			if(StaffHTML[x].indexOf('href="tel:') >= 0) {
				aarepphn[arrIndex] = StaffHTML[x].split('href="tel:')[1];
				aarepphn[arrIndex] = aarepphn[arrIndex].split('"')[0];
			}
			else {
				aarepphn[arrIndex] = "";
